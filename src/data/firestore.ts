...
import type { Lifecycle, Template, Sla, StatusDef, NotifRule, AppNotification, ReplyTemplate, FieldDef, AccessRequest, Asset, AreaNode, RoutingStats } from '../model.js';
...
interface TenantDoc { name: string; key: string; active: boolean; branding?: Branding; summary?: TenantSummary; categories: string[]; categoryTree?: CatNode[]; classificationTree?: AreaNode[]; statuses?: StatusDef[]; picklists?: Picklists; priorityMatrix?: PriorityMatrix; businessHours?: BusinessHours; holidays?: string[]; sites?: string[]; departments?: string[]; userGroups?: string[]; roles?: RoleDef[]; notifRules?: NotifRule[]; closureRules?: ClosureRules; replyTemplates?: ReplyTemplate[]; businessRules?: BusinessRule[]; formRules?: FormRule[]; webhooks?: Webhook[]; kbArticles?: KbArticle[]; announcements?: Announcement[]; customFields?: FieldDef[]; serviceCategoryIcons?: Record<string, string>; organizateGroupIds?: string[]; serviceCategories?: ServiceCategoryDef[]; operationMode?: 'classic' | 'simplified'; classificationVersion?: 'legacy' | 'v3'; routingStats?: RoutingStats; liveRouting?: boolean; idContinuity?: boolean; reportSchedules?: import('../reports.js').ReportSchedule[]; inboundEnabled?: boolean; capacity: Record<string, Capacity>; counter: number }
...
export async function createInstance(t: TenantData): Promise<void> {
  ...
  const tdoc = {
    ...
    operationMode: 'simplified' as const, classificationVersion: 'legacy' as const, capacity: {}, counter: t.counter ?? 1000,
