...
interface TenantDoc { name: string; key: string; active: boolean; categories: string[]; categoryTree?: CatNode[]; capacity: Record<string, Capacity>; counter: number }
...
export async function seedTenantToFirestore(t: TenantData): Promise<void> {
  ...
  const tdoc: TenantDoc = { name: t.name, key: t.key, active: t.active, categories: t.categories, categoryTree: t.categoryTree ?? [], capacity: t.capacity, counter: t.counter };
  batch.set(tRef, tdoc);
...
export async function subscribeTenant(tid: string, requesterFilterUid: string | null, onData: (t: TenantData) => void): Promise<Unsub> {
  ...
  subs.push(m.onSnapshot(m.doc(db, 'tenants', tid), (d) => {
    ...
    if (t) { acc.name = t.name; acc.key = t.key; acc.active = t.active; acc.categories = t.categories ?? []; acc.categoryTree = t.categoryTree ?? []; acc.capacity = t.capacity ?? {}; acc.counter = t.counter ?? 1000; }
    emit();
  }));
