// Sistema de iconos UNIFICADO de la app: SVG de línea (estilo Feather), mismo trazo
// que el rail de navegación (fill=none, stroke=currentColor, strokeWidth=2). Sustituye
// a los emojis dispersos para dar una UI coherente y profesional. Heredan color y
// tamaño del contexto (currentColor / prop size). Los iconos de CATEGORÍA de servicio
// son contenido editable aparte (emoji o SVG subido, p. ej. Gemini) y no usan esto.
...
const PATHS: Record<string, ReactNode> = {
  ...
  // Barra superior / varios
  // Portal de plataforma: capas apiladas = varias instancias vistas desde arriba.
...
export function Icon({ name, size = 16, className, strokeWidth = 2 }: { name: string; size?: number; className?: string; strokeWidth?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden="true" style={{ flexShrink: 0, verticalAlign: 'middle' }}>
      {PATHS[name] ?? null}
    </svg>
  );
}
