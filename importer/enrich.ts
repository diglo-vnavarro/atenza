...
interface SeedTpl { id: string; name: string; type: TicketType; lifecycleId: string | null; slaId: string | null; fields: string[]; ticketCount?: number }

async function templateFieldsAndLifecycle(id: string): Promise<{ fields: string[]; lifecycleId: string | null }> {
  ...
  return { fields: [...names].map(fieldLabel), lifecycleId: lc?.id ? String(lc.id) : null };
}
...
async function main() {
  ...
  for (const tp of tpls) {
    const { fields, lifecycleId } = await templateFieldsAndLifecycle(tp.id);
    tp.fields = fields;
